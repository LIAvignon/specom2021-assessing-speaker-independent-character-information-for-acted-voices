...
then
	...
	echo "This script need the number of experiment as argument"
	exit 1
fi
...
EXP_DIR="exp/prot_zero_wofold/pv_from_iv_kaldi_ori-gressia_ori/${1}"
FEATURES_FILE="/local_disk/pegasus/laboinfo/mquillot/knowledge_distillation/exp/prot_zero_wofold/iv/1.0/teacher/masseffect_pvectors.txt"
#FEATURES_FILE="/local_disk/pegasus/laboinfo/mquillot/tklia/exp/prot_zero_wofold/p-vector-e2e-xvector_switch2/masseffect_pvectors.txt"
#FEATURES_FILE="/local_disk/pegasus/laboinfo/mquillot/vocal_similarity_system/data/masseffect_ivectors.txt"
INPUT_DIM=64
...
old_features_file=${FEATURES_FILE}
...
echo "FEATURES FILE : ${FEATURES_FILE}"
# REPLACE TRAIN AND VAL FEATURES
python3 bin/export-features.py "${EXP_DIR}/lst/train_fr.lst" "${FEATURES_FILE}" --output "${EXP_DIR}/array/train/french_feats.npy"
python3 bin/export-features.py "${EXP_DIR}/lst/train_en.lst" "${FEATURES_FILE}" --output "${EXP_DIR}/array/train/english_feats.npy"

python3 bin/export-features.py "${EXP_DIR}/lst/val_fr.lst" "${FEATURES_FILE}" --output "${EXP_DIR}/array/val/french_feats.npy"
python3 bin/export-features.py "${EXP_DIR}/lst/val_en.lst" "${FEATURES_FILE}" --output "${EXP_DIR}/array/val/english_feats.npy"


# REPLACE TEST FEATURES
echo "[replace test features] Begin"
python3 bin/export-features.py "${EXP_DIR}/lst/test_fr.lst" "${old_features_file}" --output "${EXP_DIR}/array/test/french_feats.npy"
python3 bin/export-features.py "${EXP_DIR}/lst/test_en.lst" "${old_features_file}" --output "${EXP_DIR}/array/test/english_feats.npy"
