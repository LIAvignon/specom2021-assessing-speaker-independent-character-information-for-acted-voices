...
EXP_DIR="exp/prot_zero_wofold/xv_switch"
LST_DIR="${EXP_DIR}/lst"
FEATURES_FILE="../vocal_similarity_system/data/masseffect_xvectors.txt"
TEMPERATURE="1.0"
...
python bin/train-teacher.py "${LST_DIR}/train${file_suffix}.lst" "${LST_DIR}/val${file_suffix}.lst" ${FEATURES_FILE} ${teachermdldir} --batch 12 --temperature ${TEMPERATURE} --epochs 100 --drop-rate 0.35 --emb-dim 64

python bin/embedding-extract.py $original_lst_masseffect ${ORIGINAL_FEATURES_FILE} $teachermdldir --temperature ${TEMPERATURE}

python bin/evaluate-model.py "${teachermdldir}/model_checkpoint.h5" "${FEATURES_FILE}" "${LST_DIR}/val${file_suffix}.lst" "${teachermdldir}/label_encoder.pkl" > "${teachermdldir}/eval_val.log"
